        ...
        <AnimatePresence>
          {data?.map((rate, id) => (
            <React.Fragment key={rate?.id}>
              <div className="group bg-card rounded-2xl shadow-lg border border-border p-5 flex flex-col gap-2 transition-all duration-200 hover:shadow-primary/20 hover:shadow-2xl hover:scale-[1.025] relative">
                <motion.div
                ...
                >
                  ...
                  {rate.comment && (
                    <div className="mt-3 bg-muted/80 border-l-4 border-primary/40 px-5 py-3 rounded-xl italic text-muted-foreground text-base flex items-start gap-2">
                      <motion.div
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
                        exit={{ opacity: 0 }}
