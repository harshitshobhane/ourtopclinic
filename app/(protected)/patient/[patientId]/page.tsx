...
export default async function PatientProfilePage({ params, searchParams }: PatientProfilePageProps) {
  ...
  // Fetch upcoming scheduled appointments (next 3)
  ...
  const upcomingAppointments = (appointments || [])
    .filter((a: any) => a.status === 'SCHEDULED' && new Date(a.appointment_date) > now)
      ...
      <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50 p-6">
        <div className="max-w-7xl mx-auto">
          <div className="bg-white/80 backdrop-blur-sm rounded-2xl p-6 border border-white/20 shadow-lg">
            ...
            <p className="text-gray-600 mt-2">The patient you're looking for doesn't exist or you don't have permission to view it.</p>
          </div>
        </div>
      </div>
    );
